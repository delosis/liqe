...
import { type LiqeOptions } from "../../src/types";
...
const testQuery = test.macro(
  (t, expectedResultNames: string[], options?: LiqeOptions) => {
    const matchingPersonNames = filter(parse(t.title), persons, options).map(
      (person) => {
        return person.name;
      }
    );
...
test("name:/(David)|(John)/", testQuery, [], { caseSensitive: true });
